...
const FIELD_CHECKS: { field: string; label: string; patterns: RegExp[] }[] = [
  { field: 'age', label: '나이', patterns: [/\d{2,3}\s*세/, /\d{2,3}\s*살/, /나이.*\d/, /\d{2,3}세/] },
  ...
  { field: 'family_history', label: '가족력', patterns: [/가족력|부모.*질환|아버지.*질환|어머니.*질환|가족.*병|암|당뇨|고혈압|심장|심혈관|뇌혈관|가족력.*없/] },
  { field: 'current_condition', label: '현재 건강상태', patterns: [/현재.*질환|앓고|건강.*상태|고혈압|당뇨|심혈관|뇌혈관|암.*이력|건강합니다|질환.*없/] },
  { field: 'pension_status', label: '연금/노후 준비', patterns: [/연금|퇴직연금|노후|저축|고민.*안|준비.*없/] },
  { field: 'monthly_budget', label: '월 보험료 예산', patterns: [/예산|보험료.*만원|월.*만원.*보험|보험.*\d+만/] },
];
...
export function validateStage1Completeness(messages: any[]): ValidationResult {
  ...
  for (const check of FIELD_CHECKS) {
    const found = check.patterns.some(p => p.test(allText));
    if (!found) {
